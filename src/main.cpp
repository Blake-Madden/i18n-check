...
//-------------------------------------------------
static std::pair<bool, std::wstring> read_utf16_file(const std::string& file_name)
    {
...
//-------------------------------------------------
static bool valid_utf8_file(const std::string& file_name, bool& startsWithBom)
    {
    startsWithBom = false;
...
int main(int argc, char* argv[])
        ...
        for (const auto& excFolder : providedExcFolders)
            ...
            if (fs::exists(excFolder))
                ...
                if (fs::is_directory(excFolder))
                    ...
                    // add subdirectories
                    for (const auto& p : fs::recursive_directory_iterator(folderToRecurse))
                        {
            ...
            else if (const auto relPath = fs::path{ inputFolder } / excFolder; fs::exists(relPath))
                {
    ...
    if (fs::is_regular_file(inputFolder) && fs::exists(inputFolder))
        {
        filesToAnalyze.push_back(inputFolder.string());
        }
    else if (fs::is_directory(inputFolder) && fs::exists(inputFolder))
        {
        for (const auto& p : fs::recursive_directory_iterator(inputFolder))
            {
            ...
            if (!inExcludedPath)
                ...
                for (const auto& eFile : excludedFiles)
                    ...
                    if (p.exists() && fs::exists(excFile) && fs::equivalent(p, excFile))
                        {
                        inExcludedPath = true;
                        break;
                        }
    ...
    for (const auto& file : filesToAnalyze)
        ...
        if (!readBoolOption("quiet", false))
            ...
            std::wcout << L"Examining " << ++currentFileIndex << L" of " << filesToAnalyze.size()
                       << L" files (" << fs::path(file).filename() << L")\n";
            }
        ...
        try
            ...
            else if (const auto [readUtf16Ok, fileUtf16Text] = read_utf16_file(file); readUtf16Ok)
                {
    ...
    /* Note: yes, I am aware of the irony of i18n bad practices here :)
       ...
       etc. However, I am just keeping this tool simple.*/
    ...
    for (const auto& val : cpp.get_unsafe_localizable_strings())
        {
        report << val.m_file_name << L"\t" << val.m_line << L"\t" << val.m_column << L"\t" << L"\""
               << val.m_string << L"\"\t";
    ...
    for (const auto& val : cpp.get_not_available_for_localization_strings())
        ...
        if (val.m_usage.m_type == i18n_review::string_info::usage_info::usage_type::function)
            ...
            {
            report << L"String not available for translation assigned to " << val.m_usage.m_value
                   << L"\t";
            }
    ...
    for (const auto& val : cpp.get_printf_single_numbers())
        {
        report
            << val.m_file_name << L"\t" << val.m_line << L"\t" << val.m_column << L"\t\""
            << val.m_string << L"\"\t"
