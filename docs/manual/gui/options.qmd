...
This ensures that "8859-1," "८८५९-१," and "８８５９-１" will be seen as the same number sequences.
...
**Check for inconsistent translation lengths**: select this to check for suspect lengths of translations compared to their source strings.

The default is 400%, meaning that a translation can be four times longer than the source string before emitting a warning.
This value can be changed by adjustsing the **Translation length threshold** slider.

::: {.notesection data-latex=""}
Setting this to 0% will instruct the program to warn if any translation is longer than the source string.
This is useful for legacy software where resource-loading routines are using hardcoded string lengths.
:::
